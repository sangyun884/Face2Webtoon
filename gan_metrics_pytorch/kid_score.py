...
def get_activations(files, model, batch_size=50, dims=2048,
                    cuda=False, verbose=False, img_size = 96):
    ...
    for i in tqdm(range(n_batches)):
        ...
        if is_numpy:
            ...
            images = [np.array(Image.open(str(f)).resize((img_size,img_size), Image.BILINEAR)) for f in files[start:end]]
...
def _compute_activations(path, model, batch_size, dims, cuda, model_type, img_size = 96):
    if not type(path) == np.ndarray:
        import glob
        imgs = []
        for dirpath, dirnames, filenames in sorted(os.walk(path)):
            jpg = glob.glob(os.path.join(dirpath, '*.jpg'))
            png = glob.glob(os.path.join(dirpath, '*.png'))
            imgs += jpg
            imgs += png
        if len(imgs) > 50000:
            import random
    ...
    if model_type == 'inception':
        act = get_activations(imgs, model, batch_size, dims, cuda, img_size = 96)
    elif model_type == 'lenet':
        act = extract_lenet_features(imgs, model)
    return act


def calculate_kid_given_paths(paths, batch_size, cuda, dims, model_type='inception', img_size=96, act_true = None):
    ...
    if not isinstance(act_true, np.ndarray):
        act_true = _compute_activations(pths[0], model, batch_size, dims, cuda, model_type, img_size = img_size)
    else:
        print("Not calcuated act_true again")
    pths = pths[1:]
    results = []
    for j, pth in enumerate(pths):
        print(paths[j+1])
        actj = _compute_activations(pth, model, batch_size, dims, cuda, model_type, img_size = img_size)
        kid_values = polynomial_mmd_averages(act_true, actj, n_subsets=5)
        results.append([paths[j+1], kid_values[0].mean(), kid_values[0].std()])
    results[0].append(act_true)
    return results
...
def polynomial_mmd_averages(codes_g, codes_r, n_subsets=50, subset_size = 50,
...
if __name__ == '__main__':
    ...
    results = calculate_kid_given_paths(paths, args.batch_size, args.gpu != '', args.dims, model_type=args.model, img_size = args.img_size)
    for p, m, s, _ in results:
